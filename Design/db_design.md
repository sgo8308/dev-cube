...
### 식별관계 vs 비식별관계
    ...
    식별관계의 가장 큰 장점은 무결성에 도움이 된다는 점이다.
    
    일대다 관계에서 식별관계를 갖게 되면 자식 테이블은 부모 테이블의 기본키와 자신의 칼럼 중 하나를 선택해서 복합 기본키를 만들어야 한다.
    
    예를 들어 자동차라는 부모 테이블이 있고 바퀴라는 자식 테이블이 있다고 해보자.
    
    또 바퀴 테이블에 레코드는 자동차 레코드 하나당 4개(앞좌우, 뒤좌우)만 존재 가능하다는 조건이 있다고 해보자.
    
    이 때 자동차 id와 바퀴 위치를 묶어서 기본키로 만들게 되면 이 테이블은 항상 동일한 차의 동일 위치의 자동차 바퀴가 2개 이상 들어갈 수 없도록 DB 수준에서 제한할 수 있다.  즉 무결성에 도움을 준다.
    
    하지만 일대일 관계에서는 비식별관계에서도 외래키에 not null 제약 조건을 넣게 되면 식별관계와 무결성 측면에서 별 차이가 없다.

    
    - 식별관계의 단점 
    
    위와 같이 식별관계에서 기본키는 무결성을 위해 비즈니스 의미가 있는 자연키 칼럼을 조합하는 경우가 많다. 이 자연 키 칼럼이 손자 테이블에까지 전파되면 비즈니스 요구사항 변경에 대응하기 힘들어진다.
    
    또 부모 테이블에 의존적이기 때문에 테이블이 유연하지 못하다. 만약 바퀴를 먼저 만들어야 하는 요구사항이 생기면 식별관계로는 불가능하다.
    
    식별관계의 단점이 장점보다 크기 때문에 보통은 비식별관계를 주로 쓰고 비식별관계이기 때문에 떨어지는 무결성은 비즈니스 로직에서 보충한다.
    
    ---
    
    [https://deveric.tistory.com/108](https://deveric.tistory.com/108)
