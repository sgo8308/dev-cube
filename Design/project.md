                ...
                - access token의 만료 시간을 짧게 주고 만료 시간이 긴 refresh token을 같이 사용한다.
                    ...
                    - 어떻게  탈취 당한 사실을 알아낼까?
                        1. Refresh Token은 한 번만 사용하게하고 매번 Refresh Token도 재발급해주기 (RTR 방식)
                            ...
                            해커가 Refresh Token을 탈취한 후 사용하려하면 이미 한 번 사용된 Refresh Token이 재사용되는 것이므로 탈취당했다는 사실을 알 수 있다. 
...
## OAuth2
### OAuth2란 무엇이고 무슨 문제를 해결해줄까?
    
    나의 서비스, 사용자, 사용자가 사용하는 다른 서비스가 있다고 해보자.
    
    나의 서비스에서 사용자 대신에 사용자가 사용하는 다른 서비스에 작업을 하고 싶을 수가 있다. 
    예를 들면 내 서비스에 사용자가 A라는 작업을 하면 사용자의 구글 캘린더에 기록해주는 것이다. 이럴 땐 어떻게 해야할까?
    
    가장 쉬운 방법은 사용자로부터 ID Password를 받아서 작업을 하는 것이다. 
    하지만 이 방법은 사용자가 원치 않을 확률이 높을 뿐더러 다른 서비스의 id와 password를 내 서비스에서 보관하는 것은 보안적으로도 부담이 된다. 
    또한 id password를 알 경우 모든 작업이 다 가능하기 때문에 사용자가 원치 않는 작업까지 하게 될 수 있어 더더욱 사용자의 무한한 신뢰가 없다면 불가능한 방법이다.
    
    이런 상황을 OAuth가 해결해준다. OAuth를 사용하면 사용자가 사용하는 다른 서비스에서 AccessToken을 발급한다. 
    AccessToken은 사용자가 원하는 부분만큼만 권한을 가지고 있으며 내 서비스는 이 AccessToken을 이용해서 구글 캘린더와 같은 서비스에 접근해서 작업할 수 있다.
...
### 파일 저장 경로는 어떻게 하는 것이 좋을까?
    
    처음에는 /products/{productId}/image_file.jpg 와 같은 형태가 낫다고 생각했다.
    
    왜냐하면 실제 상품의 경로와 동일하기 때문에 이미지 파일의 경로를 쉽게 찾아낼 수 있다고 생각했기 때문이다.
    
    그러나 상품 업로드 요청과 상품 이미지 업로드 요청을 분리하게 되고 상품 이미지업로드를 먼저해야하기 때문에 이미지를 업로드하는 순간에 해당하는 상품의 id를 알 수가 없어졌다. 
    
    따라서 /products/image_file.jpg 방식으로 진행하기로 결정했다.
    
### 상품 업로드 시 상품 정보 업로드와 상품 이미지 업로드를 동일한 요청에서 해결해야 할까?
    
    처음에는 동시에 해야 한다고 생각했다. 왜냐하면 이미지 파일 업로드와 상품 정보 저장 중 하나의 요청이 실패했을 때 둘 다 롤백이 되어야 한다고 생각했기 때문이다. 
    두 요청을 따로 보낼 경우 이 롤백을 구현하기가 어렵다 생각했다.
    
    그러나 파일 업로드를 롤백시키는 작업에서 롤백이 실패하는 경우와 같이 고려해야 할 사항이 많았고, 이를 해결하기 위해 주기적으로 DB와 레퍼런스가 없는 파일에 대한 삭제 작업을 배치로 진행하는 방법이 나왔다.
    
    그런데 이렇게 삭제 작업을 배치로 진행 할 경우, 두 작업을 굳이 하나의 요청에서 해결할 필요가 없다. 먼저 이미지 업로드 수행하고 이 작업이 정상적으로 완료된 경우만 상품 정보 업로드를 수행하게 한다. 
    이 때 상품 정보 업로드가 실패하더라도 이미 업로드된 이미지에 대해서는 신경쓰지 않아도 된다. 어차피 배치 작업을 통해 지워질 것이기 때문이다.
    
    따라서 두 작업을 나누어 진행하기로 결정했다. 왜냐하면 추후 MSA 전환시 이미지 처리하는 서버를 따로 분리해내기도 쉽고, 나누었을 경우 상품 정보 업로드 메소드가 책임을 하나만 갖게 되어 코드가 깔끔해진다. 
    요청을 두 번 보내야 해서 생기는 약간의 레이턴시는 등록 작업의 특성상 무시해도 될만한 수준이라고 생각한다.

### DB에 이미지 url을 저장할까 이름만 저장할까?

      - 저장 방식

      DB에 저장할 수 있는 방식은 3가지 방식이 있다.

        1. full path 저장 → https://image.gola.com/market-gola/products/image.jpg
        2. root relative path 저장 → products/image.png
        3. 이름만 저장 → image.png
      ...
      주로 고려한 사항은 다음과 같다.
        ...
        1. 저장 폴더 변경에도 유연한지 ex /products → /goods
        2. 성능적으로 문제가 없는지

      이미지 저장 스토리지 변경에 유연한지도 처음에 고려했지만, 파일 스토리지 설정을 통해 가상 domain을 설정 할 수 있고, 추후 스토리지가 변경되어도 항상 같은 가상 domian을 사용하면 1,2,3번 모두 스토리지 변경에 유연하게 대처할 수 있기 때문에 고려사항에서 제외했다.

      - 생각 과정

      이름만 저장하는 방식이 가장 좋아 보인다. 왜냐하면 1,2번 고려 사항을 모두 만족하기 때문이다.

      상품 조회를 하게 된다면 상품 서비스를 통해 저장된 이미지 이름을 갖고 온 후 이미지를 담당하는 서비스를 통해 full path를 완성해서 리턴하면 된다. 추후 domain이나 저장 폴더명이 변경되어도 이미지 서비스의 로직 또는 설정을 변경해주면 된다.

      나머지 두 방식은 모두 폴더명 변경을 하려면 해당 테이블에 모든 데이터를 업데이트 해주어야 하는 만큼 1번 고려 사항을 충족하지 않는다고 판단했다.

      **MSA에서도 이 방식이 가장 좋을까?**

      MSA 서비스에서는 이름만 저장하는 방식을 사용하게 된다면 full path를 완성하기 위해 2가지 방식을 사용할 수 있다.

      첫번째는 이미지 서비스에 요청을 하여 full path를 완성하는 방식이다. 하지만 이 방식은 추가적으로 네트워크를 타기 때문에 성능이 떨어질 수 있고, 이미지 서비스에 장애가 난다면 장애가 전파될 수 있다.

      두번째는 각 서비스가 자신의 폴더명을 들고 있는 방안이다. 만약 폴더명을 변경하게 되면 각 서비스에서 자신이 들고 있는 폴더명을 변경해주어야 한다. 폴더명 변경에 대한 책임이 분산되고, 모놀리틱 서비스보다 폴더명 변경에 대한 유연함이 떨어진다. 하지만 그럼에도 불구하고 1번 또는 2번 방식을 선택해서 변경마다 모든 데이터를 업데이트 해주는 것보다는 변경에 유연하다고 판단된다.

      - 결론

      저장된 이미지의 이름만 저장하는 방식이 어떤 상황에서도 고려사항을 모두 만족한다고 판단되기 때문에 이 방식을 선택한다.
    
### 이미지 이름으로부터 full URL을 만드는 로직을 ProductService에 위치시켜야 할까?
    
    만약 ProductService에 위치시킨다면 DisplayProduct로부터 mainImageName을 getter로 꺼내서 작업해야 한다. 
    
    getter로 값을 얻어 작업하는 방식이 지양하는 것이 좋다. 왜냐하면 ProductService가 DisplayProduct의 세부적인 사항에 의존하게 되기 때문에 변경이 어려워진다. 여기서는 mainImageName의 변수명을 바꾸기도 어려워지고 type도 바꾸기 어려워진다.
    
    따라서 이 작업은 DisplayPrduct에 위치시키는 것이 좋다. 한편 이 작업은 추후 Review같은 곳에서 후기 사진에 대한 URL을 만들 때도 필요하므로 실제 구체적인 작업은 ImageUrlUtil이라는 유틸 클래스를 새로 만들어 여기세 위치시키도록 한다.
    
    최종적으로 다음과 같은 형태로 URL이 만들어지게 된다.
