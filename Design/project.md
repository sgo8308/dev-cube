...
### DB에 이미지 url을 저장할까 이름만 저장할까?
    ...
    1. full path 저장 → https://kr.object.ncloudstorage.com/market-gola/products/image.jpg
        - 설명
            이미지 저장 후 접근 URL을 받아서 상품 정보 업로드 때 그대로 함께 저장한다. 추후 상품 조회를 하게되도 추가적인 작업 없이 URL을 그대로 가져오면 된다. 
            
        - 장점
            1. 상품 정보 저장과 조회가 간편하다.

        - 단점
            1. 상품 저장 domain을 바꾸려면 저장된 이미지 URL을 모두 바꾸어주어야 한다. 따라서 변경에 유연하지 않다.
            2. 중복되는 URL로 저장 공간을 상대적으로 많이 차지하게 된다.
    2. root relative path 저장 → products/image.png
        - 설명
            이미지 저장 후 접근 URL에서 domain을 빼고 전달한다. 추후 상품 조회를 하게되면 서비스 로직에서 domain을 붙여서 반환한다.
            
        - 장점
            1. 상품 저장 domain이 바뀌어도 기존 저장된 것들을 변경할 필요가 없어 변경에 유연하다.

        - 단점
            1. 여러 서비스에서 상품에 접근할 경우 각 서비스마다 domain 이름을 들고 있어야 한다. domain이 변경된다면 모든 서비스에 변경을 전파해야 한다. 
                따라서 상품에 접근하는 서비스가 하나면 상관 없지만 여러 서비스라면 변경에 유연하지 못하게 된다.
            2.  위 방식을 사용하지 않을 경우 매번 조회할 때마다 이미지를 담당하는 서비스를 거쳐서 full path를 완성시켜야 하고 이는 성능에 문제가 있을 수 있다.
            3. 폴더명을 products에서 goods로 바꾸는 것처럼 변화가 생긴다면 DB 저장된 데이터를 모두 변경해주어야 한다.
    3. 이름만 저장 → image.png
        - 설명
            이미지 저장 후 저장된 이름만 반환한다. 추후 상품 조회를 하게되면 서비스 로직에서 full path를 완성해서 반환한다.
            
        - 장점
            1. 변화에 유연하다. 도메인이 바뀌든 폴더가 바뀌든 기존 데이터를 변경할 필요가 없다.
            2. DB 저장 공간을 덜 차지한다. 

        - 단점
            1. 2번 방식과 동일한 단점을 가지고 있다.
    
    결론
    
    가장 좋은 방식은 처음에 하나의 URL을 정해두고 (ex. www.image.gola.com/products/image.png) 이 후 domain이나 폴더명이 바뀔시 바뀐 곳으로 redirect하는 방식이다.
    
    이렇게 할 경우 변경에 유연하면서 성능적으로도 나쁘지 않아 보인다. 그러나 이렇게 redirect 하는 방식이 Amazon s3로는 가능한 것으로 보이나
    현재 사용하는 ncloud의 object storage로는 가능한 방법을 찾지 못했다. 또 추후 Storage를 변경하게 된다면 그 Storage에서 이 방식이 가능할지 알 수 없으므로 이 방법은 적용하지 않기로 했다.
    
    대신에 폴더명까지는 아니더라도 저장된 이미지의 domain을 원하는 domain으로 받는 것은 Amazon S3나 ncloud나 가능하고, 
    이 기능은 다른 Storage 또한 제공할 것으로 판단된다. 따라서 이미지를 서비스할 대표 도메인으로 image.gola.com을 사용하고 이에 따른 full path(www.image.gola.com/products/image.png)를 저장하는 방식을 선택했다. 
    이 경우 domain 변경에 유연하면서 성능적으로도 문제가 없다.
