...
### 파일 업데이트 시 덮어씌우는게 아니라 업로드 후 삭제 방식을 쓰는 이유와 삭제 로직을 다른 트랜잭션에 포함한 이유?
    1. 파일 업데이트 시 덮어씌우는게 아니라 업로드 후 삭제 방식을 쓰는 이유
        
        덮어씌울 경우 롤백을 위해 원본 데이터를 먼저 다운로드해야 한다. 그후 롤백할 때는 다운로드한 데이터를 다시 업로드해야 하는데 이 작업이 번거롭고,
        이 작업 과정 중에 에러가 생기는 것을 처리하기도 곤란하다. 만약 에러가 나서 2개의 이미지 중 하나는 다시 원래 이미지로 롤백했는데 하나는 롤백하는데 실패한다면 수정에 실패했는데도 불구하고 수정이 되어 있는 문제가 생긴다.
        
        반면 업로드 후 삭제 방식을 쓰면 롤백은 업로드 된 이미지를 삭제하기만 하면 된다. 그리고 삭제에 실패하더라도 실제 서비스에 영향을 주지는 않는다. 
        삭제 실패한 이미지는 언제든지 백그라운드에서 삭제 작업을 하면 된다.
        
    2. 삭제 로직을 다른 트랜잭션에 포함한 이유
        
        새로운 이미지가 업로드 되는데까지 완료되면 일단 정상적으로 수정작업을 완료할 수 있다. 기존 이미지를 삭제하는 작업은 이후에 언제든 진행해도 상관없는 작업이다.
        
        만약 기존 이미지를 삭제하는 작업을 같은 트랜잭션에 포함시켜서 이 작업에서 에러가 났을 때도 모두 롤백하게 된다면 1번에서 생기는 여러 번거로운 문제가 생긴다.